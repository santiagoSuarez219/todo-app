...
**Hallazgos:** Criterios 1 y 3 confirmados desde la primera ejecución: `create_activity` acepta y persiste `deferUntil`; `update_activity` con `deferUntil: null` limpia el campo correctamente. **Criterio 2 falló en la primera ejecución** de esta ronda (`create_recurring_activity` aceptaba `deferUntil` en silencio, sin error, por falta de `.strict()` en el schema Zod — causa raíz documentada en `spec/backlog.md`). Corregido en `backend/src/mcp/mcp.service.ts` (`server.tool` → `server.registerTool` con `z.object({...}).strict()`, alcance quirúrgico aprobado por el usuario, limitado a `create_activity` y `create_recurring_activity`). Re-ejecutado el criterio 2 con el input original del caso tras el fix: la tool ahora responde `MCP error -32602: Unrecognized key: "deferUntil"`, sin crear ninguna plantilla. Suite completa re-confirmada por `@tester` sin fallos nuevos (unit 105/105, e2e 151/154, únicos 2 fallos preexistentes no relacionados).

---

## Resumen de la ronda
- Aprobados: 13 (TC-030-001, 003, 004, 005, 006, 007, 008, 009, 011, TC-MCP-030-001, 002, 003, 004) — Fallidos: 0 — Pendientes: 2 (TC-030-002 diferido por caer domingo, TC-030-010 diferido a que el cron corra)
- Hallazgos escalados a `spec/backlog.md`: (1) `EmptyState` no distingue "vacío de verdad" de "todo diferido" (TC-030-011, sin corregir, hallazgo de copy/UX); (2) `create_recurring_activity` no rechazaba `deferUntil` — descartado silenciosamente por falta de `.strict()` en el schema Zod (TC-MCP-030-004) — **corregido** en `backend/src/mcp/mcp.service.ts`, re-verificado en verde tras el fix (alcance quirúrgico: solo `create_activity`/`create_recurring_activity`, el resto de tools de escritura queda documentado como deuda técnica pendiente de evaluación)
